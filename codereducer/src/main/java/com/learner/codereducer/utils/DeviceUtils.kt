...
object DeviceUtils {
    ...
    fun getUniqueID(mContext: Context): String {
        var m_szDevIDShort = ("" + getCarrierName(mContext)
                ...
                + "" + getIPAddress(true)
                + "" + getIPAddress(false))
        m_szDevIDShort = m_szDevIDShort.lowercase(Locale.ROOT)
        var m_szUniqueID = ""
        ...
        } else m_szUniqueID = generateLongString(m_szDevIDShort)
        if (m_szUniqueID.length > 80) m_szUniqueID = m_szUniqueID.substring(0, 79)
        return m_szUniqueID
    }
